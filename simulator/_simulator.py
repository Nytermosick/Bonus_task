    ...
    def run(self, time_limit: Optional[float] = None) -> None:
        ...
        try:
            ...
            while (not viewer or viewer.is_running()):
                ...
                self.times.append(t)
                self.real_state.append(pin_model.inertias[5].toDynamicParameters())
                ...
                # Step simulation
                self.controls.append(tau[0])
                self.estimated_state.append(tau[1].flatten())
                self.pos_err.append(tau[2])
                self.step(tau[0])
                p_hat = tau[1]
            ...
            self.pos_err = np.array(self.pos_err)
            self.real_state = np.array(self.real_state)
            self.estimated_state = np.array(self.estimated_state)
